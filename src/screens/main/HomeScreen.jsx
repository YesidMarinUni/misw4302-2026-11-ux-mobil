...
export default function HomeScreen() {
  const { alarms, editAlarm, createNewAlarm } = useAlarm();
  ...
  useEffect(() => {
    setTogglePreview(prev => {
      ...
      alarms.forEach(alarm => {
        next[alarm.id] = prev[alarm.id] ?? alarm.enabled;
      });
      return next;
    });
  }, [alarms]);
    ...
    <View style={{ flex: 1, backgroundColor: t.colors.bg }}>
      ...
      >
        ...
        <View style={{ gap: 12, marginBottom: 20 }}>
          {alarms.map(alarm => {
            const previewEnabled = togglePreview[alarm.id] ?? alarm.enabled;
            ...
            return (
              <TouchableOpacity
                ...
                style={{
                  ...
                  opacity: previewEnabled ? 1 : 0.5,
                  shadowColor: previewEnabled ? t.colors.text : 'transparent',
                  shadowOffset: { width: 5, height: 5 },
                  shadowOpacity: previewEnabled ? 1 : 0,
                  shadowRadius: 0,
                  elevation: previewEnabled ? 5 : 0,
                }}
              >
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <Toggle enabled={previewEnabled} onToggle={() => setTogglePreview(prev => ({ ...prev, [alarm.id]: !previewEnabled }))} />
                </View>
              </TouchableOpacity>
            );
          })}
